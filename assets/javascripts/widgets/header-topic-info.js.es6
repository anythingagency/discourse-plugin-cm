  ...
  html(attrs, state) {
    ...
    if (loaded || category) {
      if (
        ...
        let url = this.siteSettings.url ? this.siteSettings.url : Discourse.getURL("/c/") + Discourse.Category.slugFor(category);
        let icon = iconHTML('chevron-left');
        let content = [];
        content.push(iconNode("chevron-left"));
        content.push(h("span", {}, 'Back'));
        title.push(h(
          "a",
          { attributes: { href: url, class: 'title-back'} }, content,
        ));
      }
